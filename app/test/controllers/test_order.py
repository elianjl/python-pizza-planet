import pytest
from app.controllers import (IngredientController, BeverageController, 
...
def __create_sizes_and_ingredients(ingredients: list, beverages: list,  sizes: list):
...
def test_create(app, ingredients, beverages, size, client_data):
    ...
    for param, value in order.items():
        ...
        for item in created_order['detail']:
            ...
            if item['beverage_price'] is not None:
...
def test_calculate_order_price(app, ingredients, beverages, size, client_data):
    ...
    total_price = created_size['price'] + sum(ingredient['price'] for ingredient in created_ingredients) + sum (beverage['price'] for beverage in created_beverages)
